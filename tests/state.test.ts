...
let runtime = new YMSPRuntime({ stateFilePath: "/isolated/state.json" });
...
beforeEach(() => {
  runtime = new YMSPRuntime({ stateFilePath: "/isolated/state.json" });
  vi.clearAllMocks();
...
test("state cleanup uses IDs, preserving known counts and adding new spaces", async () => {
  ...
  expect(await readState(runtime)).toEqual({
    99: { numMasterWindows: 5 },
    100: { numMasterWindows: 1 },
  });
});
test("state is written to a temporary file then atomically renamed", () => {
  writeState(runtime, { 99: { numMasterWindows: 3 } });
...
test("AeroSpace keeps transient workspace settings in a separate file", async () => {
  ...
  runtime = new YMSPRuntime({
    ...
    stateFilePath: "/isolated/state.aerospace.json",
  });
  mocks.read.mockReturnValue(
