...
export function getTopWindow(this: WindowsManager, windows: Window[]): Window | undefined {
  if (windows.length === 0) {
    return undefined;
  }
...
export function isTopWindow(this: WindowsManager, windows: Window[], window: Window) {
  return this.getTopWindow(windows)?.id === window.id;
}

export function getBottomWindow(this: WindowsManager, windows: Window[]): Window | undefined {
  if (windows.length === 0) {
    return undefined;
  }
...
export function isBottomWindow(this: WindowsManager, windows: Window[], window: Window) {
...
export function getTopLeftWindow(this: WindowsManager): Window | undefined {
  const leftWindows = this.windowsData.filter((window) => this.isWindowTouchingLeftEdge(window));
...
export function getTopRightWindow(this: WindowsManager): Window | undefined {
  ...
  const topWindows = this.windowsData.filter((window) => window.frame.y === lowestYCoordinate);
