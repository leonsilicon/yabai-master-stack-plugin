...
export function getDividingLineXCoordinate(this: WindowsManager): number {
  if (getConfig().masterPosition === "right") {
    ...
    if (topRightWindow === undefined) {
      throw new Error("getDividngLineXCoordinate() was called when there are no windows.");
    }
    ...
    const nonStackWindows = this.windowsData.filter((window) => !this.isStackWindow(window));
    ...
    for (let i = 0; i < eligibleWindows.length - 1; i += 1) {
      ...
      if (
        ...
        numWindowsToRightOfTopRightWindow + i + 2 >= this.expectedCurrentNumMasterWindows
      ) {
    ...
    if (topLeftWindow === undefined) {
      throw new Error("getDividingLineXCoordinate() was called when there are no windows.");
    }
