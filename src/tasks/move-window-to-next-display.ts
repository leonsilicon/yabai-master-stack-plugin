...
export const moveWindowToNextDisplay = defineTask(async () => {
  ...
  const nextFocusedDisplay = sortedDisplays[(focusedDisplayOrderIndex + 1) % sortedDisplays.length];
  const { yabaiPath } = getConfig();
  await Bun.spawn([yabaiPath, "-m", "window", "--display", nextFocusedDisplay.index.toString()])
    .exited;
});
